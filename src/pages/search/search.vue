	...
	<view class="page">
		...
		<view class="search-record">
			<view class="search-title">
				...
				<view class="iconfont icon-laji" @click="remove"></view>
			</view>
	...
	export default {
		...
		methods:{
			...
			onSearch(){
				...
				uni.navigateTo({
					url: '/pages/SearchGoodsList/SearchGoodsList?keyword=' + JSON.stringify(encodeURIComponent(this.keyword)),
				})
				}
			...
			onRecord(val){
				uni.navigateTo({
					url: '/pages/SearchGoodsList/SearchGoodsList?keyword=' + encodeURIComponent(JSON.stringify({"keyword":val})),
				})
			}
		}
	}
