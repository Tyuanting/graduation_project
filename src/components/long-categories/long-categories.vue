			...
			<view class="right">
				...
				>
					<view class="right-container" v-for="(item,index) in list" :key="index">
						<view class="category-name" :id="`cid${ item.one_id }`">
	...
	export default {
		...
		methods: {
			...
			categoryClick(category, index) {
				this.$nextTick(() => {
					this.cIndex = `cid${ category.one_id }`
					this.flag = true
				})
				this.cIndex = ''
				this.active = index;
			...
			itemClick (category, item) {
				...
				if (checked) {
					category.selectedIds[item.two_id] = item
				} else {
					delete category.selectedIds[item.two_id]
				}
